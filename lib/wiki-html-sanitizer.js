...
const TOPDATA_COMMENT_RE = /<!--\s*sow-topdata-wiki:[\s\S]*?-->/g;
...
function protectTopdataComments(html) {
  const comments = [];
  const protectedHtml = String(html || "").replace(TOPDATA_COMMENT_RE, function (comment) {
...
function sanitizeWithTopdataComments(html, options) {
  ...
  return restoreTopdataComments(
    sanitizeHtml(protectedContent.protectedHtml, options).trim(),
    protectedContent.comments
  );
}
...
function sanitizeWikiHtml(html) {
  return sanitizeWithTopdataComments(html, SANITIZE_OPTIONS);
}

function renderReadOnlyWikiHtml(html) {
  return sanitizeWithTopdataComments(html, READ_ONLY_RENDER_OPTIONS);
}
