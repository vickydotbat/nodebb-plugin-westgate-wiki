...
const wikiArticleWatch = require("./wiki-article-watch");
...
async function getWikiPage(topicId, uid) {
  ...
  const [ancestorSections, namespaceData, pagePathData] = await Promise.all([
    ...
    wikiService.getSection(topicData.cid, uid, { articleTid: topicData.tid }),
  ...
  topicData.wikiPath = await wikiPaths.getArticlePath(topicData) || wikiPaths.getLegacyArticlePath(topicData);
  topicData.westgateWikiDiscussionDisabled = discussionDisabled;
  category.wikiPath = await wikiPaths.getNamespacePath(category) || wikiPaths.getLegacyNamespacePath(category);
