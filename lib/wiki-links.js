...
async function resolveTopicByTitle(cid, pageTitle, viewerUid) {
  ...
  return wikiDirectory.resolveTopicByNormalizedTitleForViewer(parsedCid, viewerUid || 0, pageTitle);
}
...
async function findTopicByTitleInAnyWikiCategory(pageTitle, settings) {
  ...
  for (const cid of cids) {
    const topic = await resolveTopicByTitle(cid, pageTitle, 0);
    if (topic && topic.slug) {
      return topic;
    }
  }
...
async function replaceWikiLinks(content, currentCategoryId, settings) {
  ...
  const replacements = await Promise.all(matches.map(async (match) => {
    ...
    if (!targetCategory) {
      ...
      let fallbackTopic = currentCategory ? await resolveTopicByTitle(currentCategory.cid, fallbackTitle, 0) : null;
    ...
    const topic = await resolveTopicByTitle(targetCategory.cid, pageTitle, 0);
