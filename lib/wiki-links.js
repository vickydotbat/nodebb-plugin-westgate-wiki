...
function buildRedlinkMarkdown(label, category, pageTitle, forumBookIcon) {
...
function buildWikiArticleLink(label, slug, forumBookIcon) {
  const rel = nconf.get("relative_path") || "";
...
function buildWikiNamespaceLink(label, category, forumBookIcon) {
  const rel = nconf.get("relative_path") || "";
...
async function replaceWikiLinks(content, currentCategoryId, settings) {
  ...
  const replacements = await Promise.all(matches.map(async (match) => {
    ...
    if (/^ns:/i.test(rawTarget)) {
      ...
      return {
        source: match[0],
        replacement: buildWikiNamespaceLink(nsDisplayLabel, targetNsCategory, forumBookIcon)
      };
    }
    ...
    if (!targetCategory) {
      ...
      if (fallbackTopic && fallbackTopic.slug) {
        return {
          source: match[0],
          replacement: buildWikiArticleLink(displayLabel, fallbackTopic.slug, forumBookIcon)
        };
      }
      ...
      return {
        ...
        replacement: redlinkCategory ?
          buildRedlinkMarkdown(displayLabel, redlinkCategory, fallbackTitle, forumBookIcon) :
    ...
    if (!topic || !topic.slug) {
      return {
        source: match[0],
        replacement: buildRedlinkMarkdown(displayLabel, targetCategory, pageTitle, forumBookIcon)
      };
    }

    return {
      source: match[0],
      replacement: buildWikiArticleLink(displayLabel, topic.slug, forumBookIcon)
    };
  }));
