...
async function replaceWikiLinks(content, currentCategoryId, settings) {
  ...
  const replacements = await Promise.all(matches.map(async (match) => {
    ...
    if (!pathSegments.length) {
      return { source: match[0], replacement: explicitPipeLabel || labelFallback };
    }
    ...
    if (!targetCategory) {
      ...
      return {
        source: match[0],
        replacement: currentCategory ? buildRedlinkMarkdown(displayLabel, currentCategory, fallbackTitle) : displayLabel
      };
    }
