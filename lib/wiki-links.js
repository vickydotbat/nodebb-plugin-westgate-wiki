...
async function buildResolverContext(currentCategoryId, settings) {
  ...
  categoryList.forEach((category) => {
    const path = buildContextNamespacePath(category, categoryByCid, settings);
  ...
  async function getNamespacePath(category) {
    ...
    if (!namespacePathByCid.has(cid)) {
      const contextPath = buildContextNamespacePath(category, categoryByCid, settings);
...
function buildContextNamespacePath(category, categoryByCid, settings) {
  const chain = [];
  let current = category;
  while (current) {
    const cid = parseInt(current.cid, 10);
    if (!Number.isInteger(cid) || !settings.effectiveCategoryIds.includes(cid)) {
      break;
    }
    chain.unshift(current);
    const parentCid = parseInt(current.parentCid, 10);
    current = Number.isInteger(parentCid) && parentCid > 0 ? categoryByCid.get(parentCid) : null;
  }

  const segments = chain
    .filter((entry, index) => !(index === 0 && wikiPaths.getCategorySlugSegment(entry).toLowerCase() === "wiki"))
    .map(wikiPaths.getCategorySlugSegment)
    .filter(Boolean);

  return segments.length ? `/wiki/${segments.join("/")}` : "/wiki";
}
