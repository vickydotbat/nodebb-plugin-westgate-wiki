...
async function resolveTopicByTitle(cid, pageTitle, viewerUid) {
  ...
  const topic = await wikiDirectory.resolveTopicByNormalizedTitleForViewer(parsedCid, viewerUid || 0, pageTitle);
  return topic && topic.cid == null ? { ...topic, cid: parsedCid } : topic;
}

async function resolveTopicBySlugLeaf(cid, pageTitleOrSlug, viewerUid) {
  const wikiDirectory = require("./wiki-directory-service");
  const parsedCid = parseInt(cid, 10);
  if (!Number.isInteger(parsedCid) || parsedCid <= 0) {
    return null;
  ...
  const result = await wikiDirectory.resolveTopicBySlugLeafForViewer(parsedCid, viewerUid || 0, pageSlug);
  if (result.status !== "ok" || !result.topic) {
    return null;
  }
  return result.topic.cid == null ? { ...result.topic, cid: parsedCid } : result.topic;
}

async function resolveTopicByTitleOrSlug(cid, pageTitleOrSlug, viewerUid) {
  return await resolveTopicByTitle(cid, pageTitleOrSlug, viewerUid) ||
    await resolveTopicBySlugLeaf(cid, pageTitleOrSlug, viewerUid);
}
 ...
 */
async function findTopicByTitleInAnyWikiCategory(pageTitle, settings, viewerUid) {
  const cids = [...settings.effectiveCategoryIds].sort((a, b) => a - b);

  for (const cid of cids) {
    const topic = await resolveTopicByTitleOrSlug(cid, pageTitle, viewerUid);
    if (topic && topic.slug) {
      return topic;
    }
  }
...
async function replaceWikiLinks(content, currentCategoryId, settings, viewerUid) {
  ...
  const replacements = await Promise.all(matches.map(async (match) => {
    ...
    if (namespaceSegments.length) {
      const canonicalResult = await wikiPaths.resolveArticlePath(pathSegments, viewerUid);
      if (canonicalResult.status === "ok" && canonicalResult.topic && canonicalResult.path) {
        const displayLabel = resolveWikiLinkDisplayLabel(explicitPipeLabel, pageTitle, canonicalResult.topic);
        return {
          source: match[0],
          replacement: buildWikiArticleLink(displayLabel, canonicalResult.path, forumBookIcon)
        };
      }
    }
    ...
    if (!targetCategory) {
      ...
      let fallbackTopic = currentCategory ? await resolveTopicByTitleOrSlug(currentCategory.cid, fallbackTitle, viewerUid) : null;

      const isBarePageLink = !namespaceSegments.length && !/^ns:/i.test(rawTarget);
      if (!fallbackTopic && isBarePageLink) {
        fallbackTopic = await findTopicByTitleInAnyWikiCategory(pageTitle, settings, viewerUid);
      }
    ...
    const topic = await resolveTopicByTitleOrSlug(targetCategory.cid, pageTitle, viewerUid);
...
async function transformWikiPostContent(data) {
  ...
  data.postData.content = await replaceWikiLinks(data.postData.content, categoryId, settings, getViewerUid(data));
  return data;
}
