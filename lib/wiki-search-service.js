...
const MAX_LIMIT = 50;
const MAX_TOPICS_PER_NAMESPACE = 500;
...
async function getPageResults(categoryRow, query, uid) {
  ...
  const tids = await db.getSortedSetRevRange(`cid:${cid}:tids`, 0, MAX_TOPICS_PER_NAMESPACE - 1);
  ...
  const topicRows = await topics.getTopicsFields(readableTids, [
    "tid",
    "cid",
    "title",
    "titleRaw",
