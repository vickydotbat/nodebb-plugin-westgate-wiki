...
function buildCreatedNamespacePath(parentSection, category) {
  const parentPath = String(parentSection && parentSection.wikiPath || "").replace(/\/+$/, "") || "/wiki";
  const segment = wikiPaths.getCategorySlugSegment(category);
  return segment ? `${parentPath}/${segment}` : serializer.buildWikiSectionPath(category);
}
...
async function renderChild(req, res, next) {
  ...
  const trail = wikiBreadcrumbTrail.forNamespaceCreate(section);
...
async function postNamespace(req, res) {
  ...
  const wikiPath = await wikiPaths.getNamespacePath(refreshed) || buildCreatedNamespacePath(wikiResult.section, refreshed);
